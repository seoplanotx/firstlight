"""
OncoWatch — promotional deck generator.
...
def page_tag(s, idx, label):
    text(s, Inches(11.7), Inches(7.02), Inches(1.5), Inches(0.34),
         [[(f"OncoWatch  ·  {idx:02d}", 9, MUTED, False, FONT)]],
...
kicker(s, Inches(0.9), Inches(0.7), "Meet OncoWatch", TEAL_BR)
...
steps = [
    ...
    ("2", "Monitor sources", "OncoWatch queries trusted public sources on a schedule while it's open.", TEAL_BR),
...
text(s, Inches(1.25), Inches(2.45), Inches(5.0), Inches(0.6),
     [[("OncoWatch IS", 18, TEAL, True, FONT)]])
...
text(s, Inches(7.15), Inches(2.45), Inches(5.0), Inches(0.6),
     [[("OncoWatch is NOT", 18, CORAL, True, FONT)]])
...
text(s, Inches(0.9), Inches(6.45), Inches(11.5), Inches(0.6),
     [[("Every finding requires clinician review. OncoWatch helps you ask better questions — not make medical decisions.",
...
kicker(s, Inches(0), Inches(1.75), "The story behind OncoWatch", CORAL)
# center the kicker
s.shapes[-1].left = Inches(0); s.shapes[-1].width = EMU_W
s.shapes[-1].text_frame.paragraphs[0].alignment = PP_ALIGN.CENTER

text(s, Inches(1.6), Inches(2.35), Inches(10.13), Inches(3.0),
     [[("OncoWatch was built for my mom.", 30, WHITE, True, SERIF)],
      [("", 10, WHITE, False, SERIF)],
      [("We found a promising trio of drugs in the research, brought it to her doctor, "
        "and he agreed it was worth a try.", 19, RGBColor(0xD8,0xE3,0xEA), False, SERIF, True)],
      [("", 6, WHITE, False, SERIF)],
      [("She left us before she could start it — in April 2025.", 19, CORAL, True, SERIF, True)]],
     align=PP_ALIGN.CENTER, line_spacing=1.25)

text(s, Inches(1.6), Inches(5.55), Inches(10.13), Inches(1.2),
     [[("So no other family has to find what matters too late.", 18, WHITE, True, FONT)],
      [("In loving memory.  💛", 15, AMBER, True, FONT)]],
...
text(s, Inches(1.02), Inches(6.75), Inches(11), Inches(0.5),
     [[("OncoWatch is an information tool, not medical advice. Every finding requires clinician review.",
...
# ---------------------------------------------------------------- save
import os
out = os.path.join(os.path.dirname(__file__), "OncoWatch-Promotional-Deck.pptx")
prs.save(out)
